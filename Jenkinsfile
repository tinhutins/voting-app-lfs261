...
pipeline {
  ...
  environment {
        TAG_VOTE   = '1.0.70' // MAJOR.MINOR.PATCH
        TAG_RESULT = '1.0.70' // MAJOR.MINOR.PATCH
        TAG_WORKER = '1.0.70' // MAJOR.MINOR.PATCH
    }
  ...
  stages {
    ...
    stage('Deploy vote app with argo CD into k8s'){
            ...
            steps {
                ...
                sh "(git config --global user.email 'thutinski@cratis.hr' && git config --global user.name 'thutinski')"
    ...
    post {
      always {
        echo 'Building whole mono pipeline for voting app is completed. There should be new images pushed on dockerhub and deployed on k8s.'
      }
    }
}
